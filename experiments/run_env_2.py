...
import zmq
import json # Or msgpack

HOST_IP = "0.0.0.0"  # Bind to all interfaces within the container
PORT = "5555"       # Port for ZeroMQ communication

context = zmq.Context()
socket = context.socket(zmq.PUB)
# The publisher binds, subscribers connect
socket.bind(f"tcp://{HOST_IP}:{PORT}")
print(f"ZeroMQ Publisher bound to tcp://{HOST_IP}:{PORT}")
...
def publish_state(obs: dict):
    ...
    # ZeroMQ sends multipart messages: topic then data
    socket.send_string(topic, flags=zmq.SNDMORE)
    socket.send_string(message_data)
    # If using bytes (e.g., with msgpack):
    # import msgpack
    # packed_data = msgpack.packb(robot_state)
    # socket.send_string(topic, flags=zmq.SNDMORE)
...
if __name__ == "__main__":
    try:
        ...
        print("Shutting down ZeroMQ publisher.")
    finally:
        socket.close()
        context.term()
